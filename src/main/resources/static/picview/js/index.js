    ...
    magnify.prototype = {
        ...
        createDOM: function () {
            ...
            return '<div class="magnify-modal">' +
                ...
                '</div>' +
                '<div class="magnify-stage" style="transform: rotate(0deg); transform-origin: 50% 50% 0px;"><img src="" alt="" class="magnify-image"></div>' +
                '<div class="magnify-footer"><div class="magnify-toolbar">' + this.createBtn(this.settings.Toolbar, btnTpl) + '</div></div>' +
                '</div>';
        },
        createTitle: function () {
            return this.settings.title ? '<a onclick="getMessage()" ><img src="/static/images/tips.png" style="width:30px;height:30px;" /></a><div class="magnify-title" style="display:none"></div>' : '';
        },
        ...
        setImageSize: function (img, tag) {
          ...
          /*  this.$image.css({
            ...
            if(this.$stage.width()<imgWidth||this.$stage.height()<imgHeight){
                if(this.$stage.width()<imgWidth){
                    this.zoom(-imgWidth/this.$stage.width(), {x: this.$stage.width() / 2, y: this.$stage.height() / 2});
                }else{
                    this.zoom(-imgHeight/this.$stage.height(), {x: this.$stage.width() / 2, y: this.$stage.height() / 2});
                }
            }else{
                this.zoom(-0.001, {x: this.$stage.width() / 2, y: this.$stage.height() / 2});
            }
        ...
        loadImg: function (imgSrc, tag) {
            ...
            imgSrc = '/static/picview/images/b1.jpg';
...
function getMessage(){
	var id = $('.magnify-title').html();
	$.ajax({
	      url:'/common/tips?id='+id,
	      type:'POST',
	      dataType:'JSON',
	      async:false,
	      data:{},
	      success:function(data){
	    	  if(data.code==10000){
	    		  ...
	    		  if(bj==null||bj==''){
	    			...
	    			var _html='<div class="hh">名称：'+name+'</div>'
	    				+'<div class="hh">档号：'+dh+'</div>'
	    				+'<div class="hh">时间：'+sj+'</div>'
	    				+'<div class="hh">人物：'+rw+'</div>'
	    				+'<div class="hh">背景：'+bj+'</div>'
	    			/*layer.tips(_html, '.magnify-title', {
	    			  tips: 1,
	    			  time:5000,
	    			  yes: function(index, layero){
	    				...
	    				layer.open({
	    					  type: 4,
	    					  shade: 0,
	    					  content: [_html, '.magnify-title'] //数组第二项即吸附元素选择器或者DOM
	    					});
	    	  }else{
